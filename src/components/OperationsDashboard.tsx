...
import {
  ...
  Flame, Archive, ArrowRight, Store,
} from "lucide-react";
...
function CardShell({ children, className="" }: { children:React.ReactNode; className?:string }) {
  return <div className={`bg-white border border-slate-100 rounded-xl overflow-hidden shadow-sm ${className}`}>{children}</div>;
}

function CardHeader({ icon:Icon, title, badge, badgeStatus="info" }: { icon:React.ElementType; title:string; badge?:string; badgeStatus?:string }) {
  return (
    <div className="flex items-center justify-between px-4 py-3 border-b border-slate-100">
      <h3 className="text-xs font-semibold text-slate-700 flex items-center gap-2"><Icon className="w-3.5 h-3.5 text-slate-500"/>{title}</h3>
      {badge && <Pill status={badgeStatus}>{badge}</Pill>}
    </div>
  );
}

const syncColors: Record<SyncStatus,string>  = { synced:"success", delayed:"warning", failed:"danger"  };
const syncLabels: Record<SyncStatus,string>  = { synced:"Synced",  delayed:"Delayed", failed:"Failed"  };
const systemColors: Record<SystemStatus,string> = { online:"bg-emerald-400", active:"bg-emerald-400", partial:"bg-amber-400", offline:"bg-red-400" };
const systemPills:  Record<SystemStatus,string> = { online:"success", active:"success", partial:"warning", offline:"danger" };
const systemLabels: Record<SystemStatus,string> = { online:"Online", active:"Active", partial:"Partial", offline:"Offline" };
const alertBg:    Record<AlertLevel,string> = { critical:"bg-red-50 border-l-2 border-red-400",   warning:"bg-amber-50 border-l-2 border-amber-400", info:"bg-blue-50 border-l-2 border-blue-300" };
const alertDot:   Record<AlertLevel,string> = { critical:"bg-red-500", warning:"bg-amber-500", info:"bg-blue-400" };
const alertLabel: Record<AlertLevel,string> = { critical:"danger", warning:"warning", info:"info" };
const alertLabelText: Record<AlertLevel,string> = { critical:"Critical", warning:"Warning", info:"Info" };
const filterLabels: Record<OrderFilter,string> = { all:"All", pending:"Pending", backorders:"Backorders", errors:"Errors", highvalue:"High value" };
      ...
      <div className="max-w-screen-2xl mx-auto p-5 space-y-1">
          ...
          <div className="flex items-center gap-2">
            <Popover>
              <PopoverTrigger asChild>
                <button type="button" className="flex items-center gap-2 text-xs border border-slate-200 bg-white rounded-lg px-3 py-2 text-slate-600 hover:bg-slate-50 transition-colors shadow-sm">
                  <Bell className="w-3.5 h-3.5"/>
                  <span className="bg-red-500 text-white text-xs rounded-full w-4 h-4 flex items-center justify-center font-semibold">{alerts.length}</span>
                </button>
              </PopoverTrigger>
              <PopoverContent align="end" className="w-80 p-0">
                ...
                <div className="max-h-80 overflow-auto divide-y divide-slate-50">
                  {alerts.map((a,i)=>(
                    <button key={i} onClick={()=>toast(a.text)}
              ...
              <PopoverContent align="end" className="w-auto p-0">
                <div className="flex items-center justify-between gap-2 px-3 py-2 border-b border-slate-100">
                  ...
                  <div className="flex gap-1">
                    {[7,30,60].map(n=>(
                      <button key={n} onClick={()=>setRange({from:subDays(TODAY,n-1),to:TODAY})}
                        className="text-[10px] px-2 py-1 rounded border border-slate-200 text-slate-600 hover:bg-slate-50">{n}d</button>
                    ))}
                  </div>
                </div>
                <Calendar mode="range" selected={range} onSelect={setRange} numberOfMonths={2}
                  defaultMonth={range?.from??TODAY} initialFocus className="p-3 pointer-events-auto"/>
              </PopoverContent>
            </Popover>
          </div>
        </div>
        ...
        <div className="grid grid-cols-3 lg:grid-cols-6 gap-3">
          {operationsKPIs.map(k=>(
            <div key={k.label} className="relative bg-white border border-slate-100 rounded-xl p-4 shadow-sm overflow-hidden hover:shadow-md transition-shadow">
              ...
              <div className={`flex items-center gap-1 text-xs pl-2 ${k.positive===true?"text-emerald-700":k.positive===false?"text-red-600":"text-slate-500"}`}>
        ...
        <CardShell>
          <div className="flex items-center justify-between px-4 py-3 border-b border-slate-100">
            <h3 className="text-xs font-semibold text-slate-700 flex items-center gap-2"><ShoppingCart className="w-3.5 h-3.5 text-slate-500"/>Recent orders</h3>
            <button className="text-xs text-blue-600 hover:underline flex items-center gap-1">View all<ArrowRight className="w-3 h-3"/></button>
          </div>
