import { useState } from "react";import { Card } from "@/components/ui/card";import { Button } from "@/components/ui/button";import { Badge } from "@/components/ui/badge";import { Input } from "@/components/ui/input";import { Label } from "@/components/ui/label";import { Dialog, DialogContent, DialogFooter, DialogHeader, DialogTitle } from "@/components/ui/dialog";import { Table, TableBody, TableCell, TableHead, TableHeader, TableRow } from "@/components/ui/table";import { Tabs, TabsContent, TabsList, TabsTrigger } from "@/components/ui/tabs";import { Search, Plus, Pencil, Trash2, Phone, Mail, Globe, Star, Package } from "lucide-react";import { toast } from "sonner";

type Vendor = {id; name; contact; email; phone; website;location; categories; leadTime; minOrder;paymentTerms; rating; totalOrders; totalVolume; status:"Active"|"Inactive";};

const INIT_VENDORS: Vendor[] = [{ id:"1", name:"National Tire Supply",  contact:"John Barnes",   email:"john@nts.com",    phone:"(800)111-2222", website:"nts.com",         location:"Charlotte, NC", categories:["MM","LT","HP"],    leadTime:"1-2 days", minOrder:10,  paymentTerms:"Net 30", rating:4.8, totalOrders:201, totalVolume:67800,  status:"Active" },{ id:"2", name:"Cleve Tire Wholesale",  contact:"Mike Cleve",    email:"mike@cleve.com",  phone:"(800)222-3333", website:"clevetire.com",    location:"Cleveland, OH", categories:["MM","UHP","AT"],   leadTime:"2-3 days", minOrder:4,   paymentTerms:"Net 15", rating:4.6, totalOrders:142, totalVolume:48200,  status:"Active" },{ id:"3", name:"Vans Tire Pros",        contact:"Van Smith",     email:"van@vanstire.com",phone:"(800)333-4444", website:"vanstire.com",     location:"Atlanta, GA",   categories:["HP","UHP"],        leadTime:"3-4 days", minOrder:8,   paymentTerms:"Net 30", rating:4.2, totalOrders:98,  totalVolume:31500,  status:"Active" },{ id:"4", name:"Wholesale Tires Co",    contact:"Lisa Chang",    email:"lisa@wtco.com",   phone:"(800)444-5555", website:"wholesaletires.com",location:"Dallas, TX",    categories:["MM","LT","OTR"],  leadTime:"4-5 days", minOrder:20,  paymentTerms:"Net 45", rating:3.8, totalOrders:76,  totalVolume:24100,  status:"Active" },{ id:"5", name:"Southeast Tire Depot",  contact:"Bob Harris",    email:"bob@setd.com",    phone:"(800)555-6666", website:"setd.com",         location:"Birmingham, AL",categories:["MC","HPLT"],      leadTime:"2-3 days", minOrder:6,   paymentTerms:"Net 30", rating:4.4, totalOrders:55,  totalVolume:18400,  status:"Inactive"},];

const emptyForm = { name:"", contact:"", email:"", phone:"", website:"", location:"", categories:"", leadTime:"", minOrder:"", paymentTerms:"Net 30" };

export function Vendors() {const [vendors, setVendors]   = useState<Vendor[]>(INIT_VENDORS);const [search, setSearch]     = useState("");const [open, setOpen]         = useState(false);const [editing, setEditing]   = useState<Vendor|null>(null);const [form, setForm]         = useState(emptyForm);const [selected, setSelected] = useState<Vendor|null>(null);

const filtered = vendors.filter(v => {const q = search.toLowerCase();return !q || v.name.toLowerCase().includes(q) || v.location.toLowerCase().includes(q);});

const openAdd = () => { setEditing(null); setForm(emptyForm); setOpen(true); };const openEdit = (v: Vendor) => {setEditing(v);setForm({ name.name, contact.contact, email.email, phone.phone, website.website,location.location, categories.categories.join(", "), leadTime.leadTime,minOrder(v.minOrder), paymentTerms.paymentTerms });setOpen(true);};

const save = () => {if (!form.name || !form.email) { toast.error("Name and email required"); return; }if (editing) {setVendors(v => v.map(x => x.id===editing.id ? { ...x,...form,categories.categories.split(",").map(c=>c.trim()),minOrder(form.minOrder)||0 } : x));toast.success("Vendor updated");} else {const vendor: Vendor = {id.now().toString(),...form as any,categories.categories.split(",").map(c=>c.trim()),minOrder(form.minOrder)||0,rating:4.0, totalOrders:0, totalVolume:0, status:"Active",};setVendors(v => [vendor,...v]);toast.success("Vendor added");}setOpen(false);};

const deleteVendor = (id: string) => {setVendors(v => v.filter(x => x.id!==id));toast.success("Vendor removed");};

const activeVendors = vendors.filter(v => v.status==="Active").length;const totalVolume   = vendors.reduce((s,v) => s+v.totalVolume, 0);const avgRating     = (vendors.reduce((s,v) => s+v.rating, 0)/vendors.length).toFixed(1);
